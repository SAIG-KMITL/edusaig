...
type CreateCategoryFormData = z.infer<typeof CategorySchema>;
    ...
    const onSubmit = async (data: CreateCategoryFormData): Promise<void> => {
        try {
            ...
            if (response.error?.message) {
                ...
                Toast(CATEGORY.CREATE_SUCCESS, "success");
                router.push("/");
            }
            reset();
        } catch (error) {
            Toast(
                error instanceof Error ? error.message : CATEGORY.CREATE_FAILED,
                "error"
            );
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="flex justify-center w-full h-full bg-slate-300">
            <div className="flex flex-col w-full max-w-4xl h-auto m-16 p-6 space-y-4 rounded-lg bg-slate-100 shadow-xl">
                <h2 className="text-3xl font-semibold text-black">Create Category</h2>
